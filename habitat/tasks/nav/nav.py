...
class FowMap(Measure):
    ...
    def reset_metric(self, *args: Any, episode, task, **kwargs: Any):
        ...
        # Re-mapping to follow habitat format
        self._top_down_map = np.zeros_like(task.sceneMap)
        for i in range(task.sceneMap.shape[0]):
            for j in range(task.sceneMap.shape[1]):
                if task.sceneMap[i, j] == 1:  # BORDER
                    self._top_down_map[i, j] = 2
                elif task.sceneMap[i, j] == 2:  # INVALID POINT
                    self._top_down_map[i, j] = 0
                elif task.sceneMap[i, j] == 3:  # VALID POINT
                    self._top_down_map[i, j] = 1
